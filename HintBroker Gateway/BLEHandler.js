...
var BLEHandler = {
  ...
  //Function taken from Estimote-specs to parse BLE data
  parseEstimoteTelemetryPacket: function(data) { // data is a 0-indexed byte array/buffer
  ...
  startScan: function() {
    ...
    function discoverCallback(peripheral) {
      ...
      var telemetryPacket = this.parseEstimoteTelemetryPacket(data);
      if (telemetryPacket) {
        console.log(telemetryPacket);
        this.emitter.emit('data', telemetryPacket);
      }
    }
    noble.on('discover', discoverCallback.bind(this));
